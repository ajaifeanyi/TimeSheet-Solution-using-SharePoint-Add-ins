...
SubstituteApproverList = function () {
    var c = function () {
        ...
        , e = function (h, g, f) {
            var j;
            var i = appweburl + "/_vti_bin/ListData.svc/SubstituteApprovers/?$inlinecount=allpages&$select=Id,Manager,SubstituteApprover,StartDate,EndDate1,IsActive&$expand=SubstituteApprover,Manager" + (isEmpty(f) ? "" : "&$filter=ManagerId eq " + f) + "&$orderby=" + g.jtSorting.replace(" DESC", " desc").replace(" ASC", " asc") + "&$skip=" + g.jtStartIndex + "&$top=" + g.jtPageSize;
    ...
    $("#SubstituteApprovers").jtable({
        ...
        fields: {
            ...
            IsActive: {
                ...
                width: "20%",
                display: function (e) {
                    return (e.record.IsActive != null && e.record.IsActive == true) ? "Yes" : "No"
                }
            },
            CustomEditAction: {
                title: "",
