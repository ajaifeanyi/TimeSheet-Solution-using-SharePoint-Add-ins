...
Default = function () {
    var c = function (f, e) {
        ...
        var d = rowLimiterFilter + " RequesterId eq " + CurrentUser.Id + " and ((Status eq 'Draft' or Status eq 'PendingApproval') or  (Created ge datetime'" + i + "T00%3a00%3a00')   )";
        var g = appweburl + "/_vti_bin/ListData.svc/Requests/?$filter=" + d + "&$inlinecount=allpages&$select=Id,Status,Created,FirstDayOfPeriod,Period,Year,TotalHours&$orderby=" + e.jtSorting.replace(" DESC", " desc").replace(" ASC", " asc") + "&$skip=" + e.jtStartIndex + "&$top=" + e.jtPageSize;
        ...
        , b = function (e, f) {
            var d;
            $.ajax({
                url: appweburl + "/_vti_bin/ListData.svc/Requests/?$select=Id,Status&$filter=" + rowLimiterFilter + " CreatedById eq " + CurrentUser.Id + " and WeekNumber eq '" + e + "'  and Year eq '" + f + "'",
                type: "GET",
                async: false,
                cache: false,
                headers: {
        ...
        , a = function (d, f) {
            var e;
            $.ajax({
                url: appweburl + "/_vti_bin/ListData.svc/Requests/?$select=Id,Status&$filter=" + rowLimiterFilter + "CreatedById eq " + CurrentUser.Id + " and Period eq '" + d + "'  and Year eq '" + f + "'",
                type: "GET",
                async: false,
                cache: false,
                headers: {
...
$(function () {
    ...
    $("#divEdit").click(function () {
        if (SystemSettings.PeriodType === "Weekly" || SystemSettings.PeriodType === "Bi-Weekly") {
            ...
            if (v !== null) {
                if (v.Status !== "Approved" || (CurrentUser.IsAdmin || CurrentUser.IsManager)) {
            ...
            if (SystemSettings.PeriodType === "Monthly") {
                v === Default.getCurrentMonthForCurrentUser(moment().format("MMMM"), moment().year());
                if (v !== null) {
                    if (v.Status !== "Approved" || (CurrentUser.IsAdmin || CurrentUser.IsManager)) {
                        location.href = "RequestFormEdit.html?requestID=" + v.Id;
                    } else {
                ...
                if (SystemSettings.PeriodType === "Semi-Monthly") {
                    ...
                    if ((moment().date() <= 15)) {
                     ...
                     v = Default.getCurrentMonthForCurrentUser(y, moment().year());
                    if (v !== null) {
                        if (v.Status !== "Approved" || (CurrentUser.IsAdmin || CurrentUser.IsManager)) {
                            location.href = "RequestFormEdit.html?requestID=" + v.Id;
                        } else {
