...
ToApproveByMe = function () {
    ...
    var getFormDigestValue = function () {
        ...
        getPendingRequests = function (postData, jtParams) {
            ...
            var filter = rowLimiterFilter + " CurrentApproverId eq '" + CurrentUser.Id + "' and Statu eq 'Pending Approval'";

            var query = appweburl + "/_vti_bin/ListData.svc/Requests/?$filter=" + filter + "&$inlinecount=allpages"
                + "&$select=Id,RequesterName,Statu,Created,Period,Year,TotalHours,NumberOfApprovers,ApprovalStatus,FirstApproverId,SecondApproverId"
        ...
        getSubstituteApproverByManagerId = function (managerId) {
            ...
            $.ajax({
                url: appweburl + "/_vti_bin/ListData.svc/SubstituteApprovers?$select=Id,SubstituteApprover&$expand=SubstituteApprover&$filter=IsActive1 eq true and ManagerId eq " + managerId + " and StartDate le datetime'" + today + "T00%3a00%3a00'  and EndDate1 ge datetime'" + today + "T00%3a00%3a00' ",
                type: 'GET',
                headers: {
                    'accept': 'application/json;odata=verbose'
                },
        ...
        getRequestForm = function (id) {
            ...
            var selectFields = "$select=ID,FirstDayOfPeriod,Created,Modified,RequesterId,SecondApproverId,FirstApproverId,RequesterName,Author/Id,Author/Title,FirstApprover/Title,SecondApprover/Title,FirstApprover/Id,SecondApprover/Id,Title,Statu,SchemaInstance,NumberOfApprovers,ApprovalStatus,EnableAttachments,CurrentApproverId,RejectReason,TimeSheetJSON,FirstApprover,SecondApprover,SheetSchemaInstance,TotalHours,WeekNumber,Year,FirstDayOfPeriod,NumberOfDays,Period,Year,ApprovalType,ProjectsEnabled,TitleEnabled,CategoryEnabled,NumberOfDecimalPlaces,BillableHours,BillingAmount,HourlyRate,BillableHoursAmount,BillingAmountTotal,DateFieldsMode" + customFieldsStr;
            ...
            if (isFinalApproval == true) {
                ...
                if (isEmpty(SystemSettings.IntegrationStatus) == false && SystemSettings.IntegrationStatus == "true" && SystemSettings.IntegrationExportTriggers.indexOf("Approved") !== -1) {
                    ...
                    if (requestForm != null) {
                        ...
                        if (isEmpty(JSONData["Statu"]) == false) {
                            JSONData["Statu"] = "Approved";
                        }
