...
SubmissionReport = function () {
    var a = function () {
        var d;
        $.ajax({
            url: appweburl + "/_vti_bin/ListData.svc/UserProfiles/?$inlinecount=allpages&$select=Id,User,UserName1,UserId,UserName1",
            type: "GET",
            headers: {
        ...
        , b = function (d) {
            var e;
            $.ajax({
                url: appweburl + "/_vti_bin/ListData.svc/UserProfiles/?$inlinecount=allpages&$select=Id,User,UserName1,UserId,UserName1&$filter=ManagerId eq " + d,
                type: "GET",
                headers: {
                    accept: "application/json;odata=verbose"
                },
        ...
        , c = function () {
            ...
            var h = "Id,Created,Modified,RequesterName,RequesterId,Id,Statu,Created,Period,Year,TotalHours,TimeSheetJSON";
...
$(document).ready(function () {
    ...
    $("#btnRefresh").click(function (i) {
        ...
        $.each(c, function () {
            this.IsCreated = (String(this.qUserId) in b ? true : false);
            if (this.IsCreated) {
                this.Created = b[this.qUserId].Created
            }
            this.Status = (this.IsCreated == true ? b[this.qUserId].Status : "");
            this.IsSubmitted = (this.IsCreated == true && isEmpty(this.Status) == false && this.Status != "Draft");
            this.TotalHours = (this.IsCreated == true ? b[this.qUserId].TotalHours : "")
        });
        var d = [];
        $.each(c, function () {
            if (t == false && u == false) {
