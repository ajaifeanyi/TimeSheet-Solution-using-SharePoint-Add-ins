...
RequestFormView = function () {
    var f = function () {
        ...
        , g = function (p) {
            ...
            var r = "$select=ID,FirstDayOfPeriod,Created,Modified,RequesterId,SecondApproverId,FirstApproverId,RequesterName,RequesterEmail,Author/Id,Author/Title,FirstApprover/Title,SecondApprover/Title,FirstApprover/Id,SecondApprover/Id,Title,Statu,SchemaInstance,NumberOfApprovers,ApprovalStatus,EnableAttachments,CurrentApproverId,RejectReason,TimeSheetJSON,FirstApprover,SecondApprover,SheetSchemaInstance,TotalHours,WeekNumber,Year,FirstDayOfPeriod,NumberOfDays,Period,Year,ApprovalType,ProjectsEnabled,TitleEnabled,CategoryEnabled,NumberOfDecimalPlaces,BillableHours,BillingAmount,HourlyRate,BillableHoursAmount,BillingAmountTotal,DateFieldsMode" + n;
            $.ajax({
        ...
        , l = function (u, n, p, t) {
            var s = {
                ...
                Statu: RequestStatusEnum.PendingApproval.Value,
            ...
            $.ajax({
                ...
                success: function (v) {
                    if (isEmpty(SystemSettings.IntegrationStatus) == false && SystemSettings.IntegrationStatus == "true" && SystemSettings.IntegrationExportTriggers.indexOf("Pending Approval") !== -1) {
                        var w = t;
                        if (isEmpty(w.Statu) == false) {
                            w.Statu = "Pending Approval"
                        }
        ...
        , m = function (p, o) {
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    Statu: RequestStatusEnum.Approved.Value,
                    SendEmailOnChange: true,
        ...
        , k = function (u, s, o, t, q) {
            ...
            var r = {
                ...
                Statu: RequestStatusEnum.Rejected.Value,
                RejectReason: s,
                SendEmailOnChange: true
            };
            if (q == true) {
        ...
        , h = function (n) {
            ...
            $.ajax({
                url: appweburl + "/_vti_bin/ListData.svc/SubstituteApprovers?$select=Id,SubstituteApprover&$expand=SubstituteApprover&$filter=IsActive1 eq true and ManagerId eq " + n + " and StartDate le datetime'" + o + "T00%3a00%3a00'  and EndDate1 ge datetime'" + o + "T00%3a00%3a00' ",
                type: "GET",
                headers: {
                    accept: "application/json;odata=verbose"
                },
        ...
        , e = function (s) {
            ...
            var p = "Id eq " + s;
            var t = "Id,Created,RequesterName,Id,Statu,Created,Period,Year,FirstDayOfPeriod,TotalHours,BillableHoursAmount,BillingAmountTotal,TimeSheetJSON" + (n.length > 0 ? "," + n : "");
...
$(document).ready(function () {
    ...
    if (RequestFormView.hasAccessToRequest(G.Author.Id, CurrentUser.Id, CurrentUser.IsAdmin, CurrentUser.IsManager, G.Statu, G.CurrentApproverId, G.RequesterId) == false) {
    ...
    for (var D = 0; D < E; D++) {
        var P = G[b[D]].Title;
        var s = D + 1;
        if (E > 1 && G.Statu == RequestStatusEnum.PendingApproval.Value) {
            P += " " + (s < a ? "(Approved)" : "(Pending)")
        } else {
            if (E > 1 && G.Statu == RequestStatusEnum.Rejected.Value) {
                if (s < a) {
                    P += " (Approved)"
                } else {
                    if (s == a) {
    ...
    if ((G.Author.Id == CurrentUser.Id || G.RequesterId == CurrentUser.Id) && (G.Statu == RequestStatusEnum.Draft.Value || G.Statu == RequestStatusEnum.Rejected.Value)) {
        $("#btnSendRequestToApprove").show();
        $("#btnSendRequestToApprove").removeAttr("disabled")
    } else {
        if (G.Statu == RequestStatusEnum.PendingApproval.Value) {
            $("#btnEditRequest").hide();
            ...
            if (G.Statu == RequestStatusEnum.Approved.Value) {
                $("#btnEditRequest").hide();
    ...
    $("#lblStatus").text(G.Statu);
    if (G.Statu == "Rejected" && G.RejectReason != null) {
        $("#pRejectReason").show();
        $("#lblRejectReason").text(G.RejectReason)
    } else {
        $("#pRejectReason").hide()
    }
    if (G.Statu == "Approved" && G.ApproveDate != null) {
        $("#pApproveDate").show();
    ...
    $("#btnExportDetailsCSV").click(function (aa) {
        ...
        var R = '","';
        var ai = '"\r\n';
        var S = '"Timesheet Id","Created","User","Period","First Day of Period","Year","Statu"';
        var V = "";
        var U = [];
        var Y = "";
        var X = [];
    ...
    $("#btnExportDetailsXLSX").click(function (aa) {
        ...
        var Q = '","';
        var ag = "";
        var T = '"Timesheet Id","Created","User","Period","First Day of Period","Year","Statu"';
        var V = "";
        var U = [];
        var Y = "";
        var X = [];
