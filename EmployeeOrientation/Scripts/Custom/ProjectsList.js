...
ProjectsList = function () {
    var b = function () {
        ...
        , c = function (e, d) {
            var g;
            var f = appweburl + "/_vti_bin/ListData.svc/Projects?$inlinecount=allpages&$select=Id,Title,IsActive&$orderby=" + d.jtSorting.replace(" DESC", " desc").replace(" ASC", " asc") + "&$skip=" + d.jtStartIndex + "&$top=" + d.jtPageSize;
        ...
        , a = function (e) {
            ...
            return $.Deferred(function (g) {
                $.ajax({
                    url: appweburl + "/_api/Web/lists/getbytitle('Projects')/getItemByStringId('" + e + "')",
                    type: "DELETE",
                    headers: {
                        accept: "application/json;odata=verbose",
                        "X-RequestDigest": d,
...
$(document).ready(function () {
    ...
    $("#Projects").jtable({
        ...
        actions: {
            listAction: ProjectsList.readAll,
            deleteAction: function (a) {
                return ProjectsList.deleteItem(a.Id);
                $("#Projects").jtable("reload")
            }
        },
        fields: {
            Id: {
            ...
            CustomEditAction: {
                ...
                width: "1%",
                display: function (a) {
                    return "<button title='Edit' onclick='location.href=\"ProjectEdit.html?ID=" + String(a.record.Id) + "\"' class='jtable-command-button jtable-edit-command-button'><span>Edit</span></button>"
                }
            }
        }
    });
    $("#Projects").jtable("load");
