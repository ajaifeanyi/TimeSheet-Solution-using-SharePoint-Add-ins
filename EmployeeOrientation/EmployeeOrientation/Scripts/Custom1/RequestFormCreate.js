...
RequestFormCreate = function () {
    var i = function () {
        ...
        , c = function () {
            var q;
            $.ajax({
                url: appweburl + "/_api/Web/lists/getbytitle('Categories')/items?$select=Id,Title,IsActive&$filter=IsActive eq 1&$orderBy=Title asc&$top=1000",
                type: "GET",
                async: false,
                headers: {
                    accept: "application/json;odata=verbose"
        ...
        , k = function () {
            var q;
            $.ajax({
                url: appweburl + "/_vti_bin/ListData.svc/Settings?$select=ID,Title,Category,ApproversIDs,NumberOfApprovers,EnableAttachments,CustomFieldsSchema,CustomSheetFieldsSchema,PeriodType,ApprovalType,ProjectsEnabled,TitleEnabled,CategoryEnabled,NumberOfDecimalPlaces,DateFieldsMode",
                type: "GET",
                headers: {
                    accept: "application/json;odata=verbose"
                },
