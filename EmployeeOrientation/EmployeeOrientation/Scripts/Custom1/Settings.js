...
Settings = function () {
    var b = function () {
        ...
        , a = function () {
            var e;
            $.ajax({
                url: appweburl + "/_vti_bin/ListData.svc/UserRoles/?&$select=EmployeeId&$expand=EmployeeId",
                type: "GET",
                headers: {
                    accept: "application/json;odata=verbose"
                },
        ...
        , c = function () {
            var e;
            $.ajax({
                url: appweburl + "/_vti_bin/ListData.svc/Settings?$select=ID,Title,Category,NumberOfApprovers,EnableAttachments,CustomFieldsSchema,CustomSheetFieldsSchema,ManagersIDs,ApproversIDs,PeriodType,WeekNumbering,ApprovalType,ProjectsEnabled,TitleEnabled,CategoryEnabled,NumberOfDecimalPlaces,BillableHours,BillingAmount,HourlyRate,CurrencySymbol,RequestOnBehalf,AppLogo,AppLogoFile,DateFieldsMode,PreventDuplicate,MaxHours,MinHours,EditingPendingApproval,EnteringFutureTimesheets,EditingApproved,DailyTotals",
                type: "GET",
                headers: {
                    accept: "application/json;odata=verbose"
                },
...
$(document).ready(function () {
    ...
    $("#Settings").submit(function (k) {
        ...
        if (l.AppLogo == "standard") {
            $("#logoleft").css("background-image", "url('" + appweburl + "logo.png')")
        } else {
