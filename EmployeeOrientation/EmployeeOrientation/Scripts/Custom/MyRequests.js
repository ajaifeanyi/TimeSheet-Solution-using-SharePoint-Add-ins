...
MyRequests = function () {
    var b = function () {
        ...
        , c = function (f, e) {
            ...
            var d = rowLimiterFilter + "RequesterId eq " + CurrentUser.Id + " and Statu eq '" + i + "'";
            var g = appweburl + "/_vti_bin/ListData.svc/Requests/?$filter=" + d + "&$inlinecount=allpages&$select=Id,Statu,Created,Period,Year,FirstDayOfPeriod,TotalHours&$orderby=" + e.jtSorting.replace(" DESC", " desc").replace(" ASC", " asc") + "&$skip=" + e.jtStartIndex + "&$top=" + e.jtPageSize;
